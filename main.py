...
async def not_valid_input(update: Update):
    await update.message.reply_text("الرقم الجامعي خاطئ .. أدخل رقما صحيحا ")
...
async def responser(update: Update, context: ContextTypes.DEFAULT_TYPE, numbers=(), html_bl=False):
    ...
    numbers.sort()
    message = await update.message.reply_text("يتم الآن إحضار النتيجة من الموقع ، انتظر قليلا 📥")
    try:
        async with aiohttp.ClientSession() as session:
        ...
        await update.message.reply_text("هنالك مشكلة ما في الموقع ، أعد المحاولة لاحقا ", quote=True)
        for task in tasks:
            task.cancel()
    finally:
...
async def one_req(number, session: aiohttp.ClientSession, recurse=0) -> dict[bytes, int]:
    ...
    url = "https://exam.albaath-univ.edu.sy/exam-civil/"
...
def get_token() -> str:
    ...
    if token == "6422430655:AAEIjACABKcHovv6-IyGmPr9dYVzP9AyYrU":
        raise Exception(
